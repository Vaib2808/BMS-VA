...
void setup() {
  ...
  dataFile = SD.open("data.txt", FILE_WRITE); // Open dataFile in setup
  if (!dataFile) {
    digitalWrite(LED_PIN, HIGH); // Turn on the LED to indicate failure
  }
}
...
void loop() {
  ...
  if (dataFile) {
    print_cells(dataFile); 
    print_SUM_OF_CELLS(dataFile);
...
void canSniff(const CAN_message_t &receivedMsg)  {
  //  can1.events()
  value3 = receivedMsg.id;
  if (receivedMsg.id == 0x1838F380) {
    dataFile = SD.open("CAN_data.txt", FILE_WRITE);
    dataFile.print("CAN ID: ");
    dataFile.print(value3, HEX);
    dataFile.print(" ");
    ::Serial.print("CAN ID:  ");
 ...
 for (int current_ic = 0; current_ic < TOTAL_IC; current_ic++)
  {
    if (datalog_en == 0)
    {
      ...
      Serial.print(", ");
      dataFile.print(" IC ");
      dataFile.print(current_ic + 1, DEC);
...
void print_SUM_OF_CELLS(uint8_t datalog_en) {
    ...
    dataFile.println("\nTotal voltage of the TSAC: ");
    dataFile.println(total_voltage, 3);
